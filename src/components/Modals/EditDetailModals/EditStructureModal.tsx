...
import { StructurePayload, useUpdateStructure } from "@/services/structure";
...
export function EditStructureModal({
  ...
  const editStructureMutation = useUpdateStructure();
  ...
  const {
    ...
    initialValues: {
      id: selectedStructure?.id ?? undefined,
      name: selectedStructure?.name ?? "",
      structure_type: selectedStructure?.structure_type ?? "",
      internal_reference_code: selectedStructure?.internal_reference_code ?? "",
      street_address: selectedStructure?.street_address ?? "",
      zip_code: selectedStructure?.zip_code ?? "",
      country: selectedStructure?.country ?? "",
      legal_entity_name: selectedStructure?.legal_entity_name ?? "",
      tax_id_vat_number: selectedStructure?.tax_id_vat_number ?? 0,
      default_currency: selectedStructure?.default_currency ?? "",
      default_language: selectedStructure?.default_language ?? "",
      time_zone: selectedStructure?.time_zone ?? "",
      default_check_in_time:
        selectedStructure?.default_check_in_time ?? TIME[1],
      default_check_out_time:
        selectedStructure?.default_check_out_time ?? TIME[2],
      image_url: selectedStructure?.image_url ?? "",
      _localFiles: selectedStructure?._localFiles ?? [],
      status: selectedStructure?.status ?? "",
    },
    ...
    onSubmit: async (values, { setSubmitting }) => {
      ...
      editStructureMutation.mutate(values, {
        onSuccess: () => {
          onOpenChange(false);
          resetForm();
        },
  ...
  const { isFormDisabled } = useFormStatus({
    isSubmitting,
    isLoading: editStructureMutation.isPending,
  });
          ...
          <form onSubmit={handleSubmit} id="structure">
            <fieldset disabled = {isFormDisabled}>
              <div>
                ...
                <div className="lg:flex-row flex flex-col items-center lg:gap-5 justify-between">
                ...
                <div className="lg:flex-row flex flex-col items-center lg:gap-5 justify-between mt-2 pb-2">
                  <div className="flex flex-col gap-1 lg:w-1/2 w-full">
                    <Label
                ...
                <div className="flex flex-col lg:flex-row items-start lg:gap-5 w-full">
                  {/* Default Currency */}
                  <div className="flex flex-col gap-1 lg:w-1/2 w-full">
                    <Label
                ...
                <div className="flex flex-col lg:flex-row items-start gap-5 w-full mt-5 lg:mt-2">
                  {/* Time Zone */}
                  <div className="flex flex-col gap-1 lg:w-1/2 w-full">
                    <Label
                      htmlFor="time_zone"
                  ...
                  <div className="flex flex-col gap-1 lg:w-1/2 w-full">
                    ...
                    <Select
                      value={values.default_check_in_time || TIME[1]}
                      onValueChange={(value) =>
                ...
                <div className="flex flex-col lg:flex-row items-start gap-5 w-full mt-5">
                  ...
                  <div className="flex flex-col gap-1 lg:w-1/2 w-full">
                    ...
                    <Select
                      value={values.default_check_out_time || TIME[2]}
                      onValueChange={(value) =>
                ...
                <div className="flex gap-4 justify-end mt-7 mb-3">
                  ...
                  >
                    ...
                    {editStructureMutation.isPending || isSubmitting
                      ? "Saving..."
                      : "Save"}
                  </Button>
                </div>
