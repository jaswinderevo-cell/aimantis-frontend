...
import { InvoicePayload } from "@/pages/Invoicing/DashboardTab";
import { useDeleteRoom, useGetAllRooms } from "@/services/rooms";
...
interface InvoiceDashboardTableProps {
...
export function InvoiceDashboardTable({
  ...
  const columns: ColumnDef<InvoicePayload>[] = [
    {
      accessorKey: "number",
      header: ({ column }) => {
        return (
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="px-3 text-black">{row.getValue("status")}</div>
      ),
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const amountStatus = row.original.paid;

        return (
          <div className="px-3 text-black flex">
            {amount} <span>{amountStatus}True</span>
          </div>
        );
      },
    },
